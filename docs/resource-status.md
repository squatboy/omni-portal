...
### 6. GitHub
등록된 repository의 최신 commit과 GitHub Actions workflow run 상태를 모니터링합니다.

* **상태 종류**:
    1. Commit: 최신 commit 1건.
    2. Workflow Run: `queued`, `in_progress`, `waiting`, `requested`, `completed` 및 `conclusion`.
* **판별 기준**:
    * commit 조회 실패는 해당 repository 수집 실패로 보고 source를 `down`으로 표시합니다.
    * workflow run이 없으면 `latestWorkflowRun: null`로 유지하고 실패로 보지 않습니다.
    * `conclusion`이 `failure`, `cancelled`, `timed_out`, `action_required`이면 `stale`로 표시합니다.
    * `status`가 `queued`, `in_progress`, `waiting`, `requested`이면 `progressing`으로 표시합니다.
    * `conclusion`이 `success`이면 `ok`로 표시합니다.
* **오류 상태**:
    * `401/403` 인증/권한 실패는 `permission_error`로 표시합니다.
    * `429` 또는 GitHub rate-limit/secondary-rate-limit 응답은 `down`으로 표시하고 `upstreamStatus`를 남깁니다.
